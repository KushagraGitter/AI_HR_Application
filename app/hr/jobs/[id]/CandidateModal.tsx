...
export default function CandidateModal({
  ...
  const [tab, setTab] = useState<"overview" | "resume" | "outreach">("overview")
  ...
  const breakdown = Array.isArray(c.criteriaBreakdown)
    ? c.criteriaBreakdown
    : []
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-2xl max-h-[85vh] rounded-2xl border border-cardborder bg-card shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-start justify-between p-5 pb-0">
          <div className="flex-1">
            ...
            <div className="flex gap-3 mt-1 text-xs text-muted">
              {c.linkedinUrl && (
                <a href={c.linkedinUrl} target="_blank" rel="noopener noreferrer" className="hover:text-accentlt transition">
                  LinkedIn
                </a>
              )}
              {c.githubUrl && (
                <a href={c.githubUrl} target="_blank" rel="noopener noreferrer" className="hover:text-accentlt transition">
                  GitHub
                </a>
              )}
            </div>
        ...
        <div className="flex gap-1 px-5 pt-4 pb-0">
          {(["overview", "resume", "outreach"] as const).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
            ...
            >
              {t === "overview" ? "Overview" : t === "resume" ? "Resume" : "Outreach"}
            </button>
          ))}
        </div>
            ...
            <>
              ...
              {c.status === "scored" && (
                <div className={`rounded-xl border p-3 text-xs ${
                  ...
                  {isEligible
                    ? `Score ${c.score} meets the threshold (${minScore}). Eligible for outreach.`
                    : `Score ${c.score} is below the threshold (${minScore}). Not eligible for outreach.`}
                </div>
              )}
              ...
              {c.availabilitySlots && c.availabilitySlots.length > 0 && c.status !== "scheduled" && (
                <div>
                  <h4 className="text-xs font-semibold text-muted uppercase tracking-wider mb-2">
                    Submitted Availability ({c.availabilitySlots.length} slots)
                  </h4>
                  <div className="flex flex-wrap gap-1.5">
                    {c.availabilitySlots.map((slot, i) => (
                      <span key={i} className="text-[10px] bg-surface border border-cardborder rounded-lg px-2 py-1 text-muted">
                        {formatSlot(slot)}
                      </span>
                    ))}
                  </div>
              ...
              {c.emailDraft ? (
                <div>
                  ...
                  {c.availabilityToken && (
                    <div className="flex items-center gap-2 mb-3">
                      <button
                        onClick={copyLink}
                        className="rounded-lg border border-cardborder px-3 py-1.5 text-xs font-medium text-muted hover:text-fg hover:bg-cardhover transition"
                      >
                ...
                <div className="rounded-xl border border-dashed border-cardborder p-6 text-center">
                  ...
                  {c.status === "scored" && isEligible && (
                    <p className="text-[10px] text-accentlt/70">
                      Use the &ldquo;Draft Outreach&rdquo; action to generate an email for this candidate.
                    </p>
                  )}
                </div>
              )}
        ...
        <div className="border-t border-cardborder px-5 py-3 flex items-center gap-2 shrink-0 bg-card">
          {c.status === "scored" && isEligible && !c.emailDraft && (
            <button
              onClick={() => onDraftOutreach(c.id)}
              disabled={actionLoading === "drafting"}
          ...
          {c.status === "scheduled" && c.icsFile && (
            <button
              onClick={() => downloadIcs(c.icsFile, c.name)}
